...
public class MachineModel {
	...
	//sets? not gets? look back on assignment just in case
	...
	public void step(){
		try{
			//double check might be wrong really confused
	...
	public void setPC(int i) {
		cpu.pc = i;
	}//Other methods incoming at later date
	...
	public MachineModel(boolean withGUI, HaltCallback cb){
		...
		ACTION.put(opcodes.get("LOD"), instr -> {
			int flags = instr.opcode & 6; // remove parity bit that will have been verified
			//Not sure if = or += come back and check/////!!!!/////
			if(flags ==0){ //direct addressing
				cpu.accum = memory.getData(instr.arg);	
			} else if(flags == 2){ // immediate addresing
				cpu.accum = instr.arg;
	...
	/*public static void main(String[] args) {
		...
		Instruction.checkParity(instr);
		model.setAccum(0);
		model.ACTION.get(instr.ofode/8).accept(instr);
