...
insert(X, node(K, L, R), node(K, L1, R)) :- X < K, insert(X, L, L1).
insert(X, node(K, L, R), node(K, L, R1)) :- X > K, insert(X, R, R1).
...
to_list(nil, []).
to_list(node(P,L,R), List) :- to_list(L, L1), to_list(R, R1), append([L1, [P], R1], List).
...
/* Problem 3 Answer: */
right_rotate(node(Parent, node(LP, LL, LR), R), node(LP, LL, node(Parent, LR, R))).
...
/* Problem 4 Tests: I had help on  Question 4 and 5 from stackoverflow.com*/
...
binaryAP(ast(Functor, LeftExp, RightExp), Oplist) :-
	binaryAP(LeftExp, LEResult),
	binaryAP(RightExp, REResult),
...
/* Problem 6 Answer: */
numAtoms([], 0).
numAtoms([H|T], C) :- atom(H), numAtoms(T,C2), C is C2 + 1.
numAtoms([H|T], C) :- numAtoms(H, C2), numAtoms(T, C3), C is C2 + C3.
/* Problem 6 Tests: */
...
/* Problem 7 Answer:  Help got from github*/
