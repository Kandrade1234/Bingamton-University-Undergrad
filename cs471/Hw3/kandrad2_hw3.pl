...
element_at(X,[X|_],0).
element_at(X, [_|T],Count) :- Count > 0, E is Count-1, element_at( X, T ,E).
...
/* Problem 11 Answer:
 I used wikipedia for help on this question. */
...
/* Problem 11 Test: (yours) */
  :- ackermann(0,0,1).  % SUCCEED
  :- ackermann(1,0,2).  % SUCCEED
  :- ackermann(0,1,2).  % SUCEED
  :- ackermann(0,0,0).   % FAIL
